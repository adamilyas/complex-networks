...
\documentclass[11pt]{article}
    ...
    \def\lt{<}
    % Document parameters
    \title{CS-E5740 Complex Networks Submission 4}
...
\textbf{Percolation theory}

``order parameter'' \(P\): fraction of nodes in the largest connected
component (LCC)

Control parameters \(f\):
\(\frac{\text{number of active links}}{\text{number of all possible links}}\)
...
\end{Verbatim}
    ...
    \section{Percolation in Erdős-Rényi (ER)
...
small number - the size of the network N is very large. - N large, p
large such that \(p(N-1) = \langle k \rangle\) stays constant
...
a function of the fraction of removed links \(f \in [0, 1]\)
